...
export default async function LessonDetailPage({ params }: LessonPageParams) {
  ...
  if (!lesson || lesson.course_slug !== slug) notFound();
  ...
  return (
    <div>
      <Link href={`/dashboard/courses/${slug}/learn`} className="text-sm font-semibold text-gold-800 underline dark:text-gold-400">
        ← Back to Lessons
      </Link>

      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        <h1 className="font-display text-xl font-semibold text-navy-900 dark:text-white sm:text-2xl">{lesson.title}</h1>
        <LessonBookmarkButton lessonId={lessonId} initiallyBookmarked={bookmarked} />
      </div>
      ...
      <div className="mt-10 flex items-center justify-between border-t border-navy-100 pt-6 dark:border-navy-700">
        {prevLesson ? (
          <Link href={`/dashboard/courses/${slug}/learn/${prevLesson.id}`} className="text-sm font-semibold text-navy-600 hover:text-gold-800 dark:text-navy-300 dark:hover:text-gold-400">
            ← {prevLesson.title}
          </Link>
        ) : <span />}
        {nextLesson ? (
          <Link href={`/dashboard/courses/${slug}/learn/${nextLesson.id}`} className="text-sm font-semibold text-navy-600 hover:text-gold-800 dark:text-navy-300 dark:hover:text-gold-400">
            {nextLesson.title} →
          </Link>
        ) : <span />}
      </div>
