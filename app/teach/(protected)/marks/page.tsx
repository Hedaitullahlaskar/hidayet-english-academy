...
export default async function MarksEntryPage() {
  ...
  return (
    <div>
      ...
      {tests.length === 0 ? (
        <EmptyState className="mt-8" icon="🔢" title="No tests yet" body="Create a test first — student attempts and scores will appear here once they take it." />
      ) : (
        <div className="mt-8 space-y-3">
          {tests.map((t: { id: string; title: string }) => (
            <div key={t.id} className="rounded-lg border border-navy-100 bg-white p-4 shadow-card dark:border-navy-700 dark:bg-navy-800">
              ...
              <EmptyState className="mt-3" icon="📭" title="No attempts yet" body="Once students take this test, their attempts will appear here for you to review and mark." />
            </div>
          ))}
        </div>
      )}
