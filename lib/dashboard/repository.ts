  ...
  return safeQuery(
    async () => {
      ...
      const scoredAttempts = (attempts ?? []).filter(
        (a: { score: number | null; tests: { total_marks: number } | null }) => a.score !== null && a.tests?.total_marks
      );
      const avgQuizPercent =
        scoredAttempts.length > 0
          ? Math.round(
              scoredAttempts.reduce(
                (sum: number, a: { score: number; tests: { total_marks: number } }) => sum + (a.score / a.tests.total_marks) * 100,
                0
      ...
      const scoredSubmissions = (submissions ?? []).filter(
        (s: { score: number | null; assignments: { max_score: number } | null }) => s.score !== null && s.assignments?.max_score
      );
      const avgAssignmentPercent =
        scoredSubmissions.length > 0
          ? Math.round(
              scoredSubmissions.reduce(
                (sum: number, s: { score: number; assignments: { max_score: number } }) => sum + (s.score / s.assignments.max_score) * 100,
                0
