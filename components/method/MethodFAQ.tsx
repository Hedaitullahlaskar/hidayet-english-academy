...
export function MethodFAQ() {
  ...
  return (
    <section id="method-faq" className="bg-white py-20 dark:bg-navy-950 sm:py-28">
      <Container className="max-w-3xl">
        <SectionHeading eyebrow="FAQ" title="Questions About How We Teach" />
        <div className="mt-12 space-y-3">
          {methodFaqs.map((faq, i) => {
            const isOpen = openIndex === i;
            return (
              <div
                key={faq.question}
                className="overflow-hidden rounded-lg border border-navy-100 bg-paper-100 shadow-card dark:border-navy-700 dark:bg-navy-900"
              >
                <button
                  className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left"
                  onClick={() => setOpenIndex(isOpen ? null : i)}
                  aria-expanded={isOpen}
                  aria-controls={`method-faq-panel-${i}`}
                >
